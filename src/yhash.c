 ...
 *---------------------------------------------------------------------------
 *! \file yhash.c
 *  \brief Simple Super Fast Table hash lookup
 *																			
 *---------------------------------------------------------------------------
 * Version                                                                  -
...
int     hash_size_raw=YHASH_DEFAULT_LOOKUP_TABLE_SIZE;
	...
	new_hash=(YHASH*)malloc(sizeof(YHASH)+(hash_size*sizeof(YHASH_OBJ *)));
...
yhash_insert_buffer_key(YHASH *hash, char *key, int keylen, void *value)
{
	...
	// Malloc element
	object=(YHASH_OBJ*)malloc(sizeof(YHASH_OBJ));

	if(object)
	{
		// Fill in object
		object->key=(char*)malloc(keylen*sizeof(char));
		if(object->key)
		{
			// copy key
...
{
	return(yhash_insert_buffer_key(hash, key,strlen(key), value));
}
